...
bibliography: '`r normalizePath("x15.bib")`'
---

```{r  include=FALSE, echo=F}
rm(list = ls())
for (x in ((length(search()) - 1):2)) {
  detach(po = x, force = TRUE)
}
options(
...
# hd(iris) |> kable()
# skim(iris)    %>% skimr::yank("numeric")
skim_without_charts(iris) %>% skimr::partition()
```
